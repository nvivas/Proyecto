...
public class Tareas_pedidos {
    ...
    public int darAlta(Pedido pedido) {
        ...
        ResultSet rs = null;
        String sql = "INSERT INTO pedido(id_pedido, id_color, cliente_id_cliente, proveedor_id_proveedor) "
                + "VALUES (?,?,?,?)";
        ...
        try {
            ...
            consulta.setInt(2, pedido.getId_color());
    ...
    public Pedido modificar(Pedido pedido) throws SQLException {
        ...
        try {
            ...
            System.out.println("modificar TP");
            MensajeExcepcion(ex);
        }
        return null;
    }
    ...
    public List<Pedido> recuperarTodas() throws SQLException {
        ...
        try {
            PreparedStatement consulta = Conexion.cnx.prepareStatement // ("SELECT p.id_pedido FROM pedido ORDER BY id_pedido");
                    ("SELECT\n"
                            + "    p.id_pedido,\n"
                            + "    cl.nombre_cli,\n"
                            + "    c.nombre_color\n"
                            ...
                            + "    pedido p\n"
                            + "INNER JOIN color c\n"
                            + "	ON c.id_color = p.color_id_color\n"
                            + "    inner join cliente cl\n"
                            + "    on cl.id_cliente = p.cliente_id_cliente\n");

            ResultSet resultado = consulta.executeQuery();
            while (resultado.next()) {
                pedido.add(new Pedido(resultado.getInt("id_pedido"),
                        resultado.getString("nombre_cli"),
                        resultado.getString("nombre_pro"),
                        resultado.getString("nombre_color")));

            }
        } catch (SQLException ex) {
            System.out.println("Recuperar TP");
            MensajeExcepcion(ex);
        }
        return pedido;
    }
